...
double distance_track(Trajectory p, Trajectory q) {
  ...
  for (int i = 0; i < p.length; ++i) {
    double tmp =
        pow((p.cod[i].x - q.cod[i].x), 2) + pow((p.cod[i].y - q.cod[i].y), 2);
    dis += sqrt(tmp);
  }
  dis /= p.length;
  return dis;
...
Graph TDM_Cons(vector<Trajectory> &TEC, double s, double lambda, double alpha,
               double beta) { //如何随机？
  Graph TG(TEC);
...
double Anony_track(double &IL, vector<Trajectory> &TEC, int k, double s,
          ...
          for (int j = 0; j < V->countV(); ++j) {
            ...
            for (int p = 0; p < Array_Link.size(); ++p) {
              if (!same_element(Array_Link[p], V->getT())) {
                Vaw temp = {V->refind(j), Array_Link[p],
  ...
  while (it != S.end()) { // k-匿名集轨迹数目<k
    ...
    string id, id_connect;
    for (int j = 0; j < S.size(); ++j) {
      if (it == S.begin() + j || (*it).countV() + S[j].countV() >=
                                     2 * k) //匿名集节点数是不是不能超过2k?
        continue;
      string t_id, t_id_con;
  ...
  double x_max = S[0].getV(0).cod[ti % S[0].getV(0).length_Tra()].x,
         x_min = x_max,
         y_max = S[0].getV(0).cod[ti % S[0].getV(0).length_Tra()].y,
         y_min = y_max;
  for (auto g : S)
